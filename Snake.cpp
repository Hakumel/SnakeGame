...
std::once_flag Snake::m_flag_snake;
...
Snake::Snake(Wall* w) : m_NodeList(), m_phead(nullptr),
                        m_pWall(w),m_predirct(RIGHT),
                        m_nextdirct(RIGHT) {}

Snake::~Snake()=default;

void Snake::initial_data_once() {
    m_data=new Snake(Wall::getdata());
}

Snake *Snake::getdata() {
    std::call_once(m_flag_snake,&Snake::initial_data_once);
    return m_data;
}
...
void Snake::addNode(unsigned x, unsigned y) {
    QLabel* newlabel = new QLabel(Wall::getdata());
...
void Snake::setdir(char dir) {
    if( (dir == LEFT&& m_predirct==RIGHT) || (dir == UP && m_predirct == DOWN) ||
        (dir == DOWN && m_predirct == UP) || (dir == RIGHT && m_predirct == LEFT) ) {
    }
    else {
        m_nextdirct=dir;
    }
